...
2. **Model Harness** (`src/apeiron/model/torch_model_harness.py`): Abstract `BaseModelHarness` providing `get_stream_dataloader()`, `get_train_dataloaders()`, `get_hist_dataloaders()`, `update_data_stream()`, `get_criterion()`, `get_optmizer()`, `model_config()` (optional; architecture hyperparameters recorded with the run), `window_inputs()` (optional; per-window data -- fetched sources or `Derived` artifacts -- the framework materializes into the dataset store before `update_data_stream()`), `make_sampler()`/`sampler_state()`/`load_sampler_state()` (deterministic shuffling that a mid-round resume can land inside), and `eval_metrics` dict. Also keeps a per-task registry for transfer metrics -- `register_task()`, `eval_past_tasks()`, `task_diagonals` -- which subclasses inherit unchanged (see `docs/tracking.md` "Transfer Metrics").
...
5. **Training** (`src/apeiron/training/continuous_trainer.py`): `ContinuousTrainer` runs outer/inner CL loops with gradient accumulation, journals each round (`cl_started`/`cl_finished` with FWT/BWT), and supports re-entering a round interrupted partway.
6. **Updaters** (`src/apeiron/training/updater/`): `BaseUpdater` with hooks `cl_preprocessing()`, `fwd_bwd()`, `update_pre_fwd_bwd()`, `update_post_fwd_bwd()`, `update_post_optimizer_call()`, `cl_postprocessing()`, plus `state_dict()`/`load_state_dict()` for memory across drift events and within a round (EWC, KFAC). Implementations: base (vanilla), jvp_reg (JVP updater -- now a first-order SAM robust update), ewc_online (EWC), kfac_online (KFAC), none (no-op).
...
10. **Experiment runs** (`src/apeiron/experiment/`): `Run` (bounded run directories allocated under `[experiment] path`/`name`, holding the original + resolved config, model record, metrics CSV, log, checkpoints, and a signature), `Journal` (append-only SQLite event log: windows, every drift check, drift events, checkpoints; `signature()` compares two runs), `model_info` (derives class/shapes/parameter counts from the model; the optional `BaseModelHarness.model_config()` hook supplies architecture hyperparameters), `determinism` (`seed_everything`, `stable_seed`, `window_generator`, `EpochSeededSampler`, RNG capture/restore), `workspace` (`Experiment` report + `python -m apeiron.experiment <path>`), `datasets` (`DatasetStore`: per-window `window_inputs()` declarations copied from local paths or fetched from `hf://`/`https://`, `.part`-then-rename, grow-only, hand-stageable; `Derived(build, recipe)` inputs are produced after fetches and rebuilt when the recipe changes, guarded by a `.origin` marker), `memmap` (`writing`/`read`/`describe` over `.npy` for data paged in on demand), and `restart` (`RestartStore` under `checkpoints`-sibling `restart/`, `[experiment] restart_interval`, `--continue-from`, SIGUSR1/SIGTERM save-and-stop). State is saved at monitoring-loop batch boundaries and after each learning step; a run interrupted inside a round re-enters it, replaying loader positions by recorded batch counts. An interrupted-and-resumed run is signature-equal to an uninterrupted one. Config-gated: no `[experiment]` section = legacy behavior. See `docs/experiment.md`.
