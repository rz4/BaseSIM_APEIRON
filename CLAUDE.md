...
10. **Experiment runs** (`src/apeiron/experiment/`): `Run` (bounded run directories under `[experiment] path`/runs/ holding resolved+original config, metrics CSV, checkpoints), `Journal` (append-only SQLite event log: window advances, every drift check unsampled, drift events, CL rounds with FWT/BWT, checkpoints; `signature()` for rerun comparison, `resume_state()` for `--continue-from`), `determinism` (seed_everything, order-independent `window_generator`), managed data residency: `ArtifactStore` (experiment-shared disk cache, index.sqlite + store/), `ResidencyManager` (materialize/evict-LRU/pin/prefetch under a soft `artifact_budget_bytes`; stall-aware in-flight waits), `sources` (hf:// datasets + local ingest adapters), and resilience: `snapshot` (full-state SnapshotManager under checkpoints/resilience/, `[experiment] snapshot_interval`), `retention` (analysis-checkpoint policies), `workspace` (multi-run report CLI, pin GC). Runs are crash-equivalent: kill -9 or SIGUSR1/SIGTERM (walltime signals) + `--continue-from` reproduces the uninterrupted run's journal signature. Config-gated: no `[experiment]` section = legacy behavior. See `docs/experiment.md`.
